...
export function useActions() {
  ...
  return {
    ...
    oviDraftReply: (original: EmailMsg, body: string, send: boolean) => {
      ...
      if (send) {
        const sent: EmailMsg = { id: uid('em'), folder: 'sent', from: 'Ovi', fromEmail: 'jordan@tellovi.io', to: original.fromEmail, subject: `Re: ${original.subject}`, body, dealId: original.dealId, personId: original.personId, dealLabel: original.dealLabel, replyToId: original.id, time: 'Just now', createdAt: Date.now() }
        const activity: Activity | undefined = original.dealId || original.personId
          ? { id: uid('act'), type: 'email', subject: `Auto-reply sent: ${original.subject}`, body: body.slice(0, 140), dealId: original.dealId, personId: original.personId, done: true, who: 'Ovi', createdAt: Date.now(), source: 'ai' }
          : undefined
    ...
    processMeeting: (id: ID, dealId: ID | undefined, personId: ID | undefined, org: string) => {
      const acts: Activity[] = [
        { id: uid('act'), type: 'note', subject: `Meeting notes — ${org}`, body: 'AI summary: deal gated on liability-cap wording; finance signs off budget by month-end if resolved this week. Champion confirmed phased rollout. Client wants retainer to cover standby units.', dealId, personId, done: true, who: 'Solar House Notetaker', createdAt: Date.now(), source: 'meeting' },
        { id: uid('act'), type: 'email', subject: 'Send liability-cap wording to Marta Lund', dealId, personId, done: false, priority: 'High', due: 'Today', who: 'Solar House Notetaker', createdAt: Date.now(), source: 'meeting' },
        { id: uid('act'), type: 'task', subject: 'Split maintenance retainer on the quote', dealId, done: false, priority: 'Medium', due: 'Tomorrow', who: 'Solar House Notetaker', createdAt: Date.now(), source: 'meeting' },
        { id: uid('act'), type: 'meeting', subject: 'Book legal walkthrough call', dealId, personId, done: false, priority: 'High', due: 'This week', who: 'Solar House Notetaker', createdAt: Date.now(), source: 'meeting' },
      ]
    ...
    approveRun: (run: import('./types').AgentRun) => {
      ...
      if (run.kind === 'draft' && run.emailTo) {
        ...
        activity = { id: uid('act'), type: 'email', subject: `Email sent (AI): ${run.title}`, dealId: run.dealId, personId: run.personId, done: true, who: 'Ovi', createdAt: Date.now(), source: 'ai' }
      } else if (run.kind === 'task' || run.kind === 'risk') {
        activity = { id: uid('act'), type: run.kind === 'risk' ? 'email' : 'task', subject: run.title.replace(/^Suggested task: /, ''), dealId: run.dealId, personId: run.personId, due: 'Today', priority: 'High', done: false, who: 'Ovi', createdAt: Date.now(), source: 'ai' }
      }
