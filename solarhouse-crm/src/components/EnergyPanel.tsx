  ...
  return (
    <div className="p-4 border-t border-divider flex flex-col gap-3.5">
      ...
      <div>
        ...
        <div className="h-2.5 rounded-full bg-control overflow-hidden"><div className="h-full rounded-full" style={{ width: `${offsetPct}%`, background: '#62E4CC' }} /></div>
      ...
      <div className="grid grid-cols-2 gap-1.5">
        <EStat v={gen} u="kWh generated" c="#62E4CC" />
        <EStat v={selfUsed} u="used on-site" c="#17B890" />
        <EStat v={exported} u="exported" c="#0E7A66" />
      ...
      <div>
        <div className="flex items-center justify-between mb-1.5">
          ...
          <div className="flex items-center gap-2.5 text-[9.5px] text-muted-b">
            <span className="inline-flex items-center gap-1"><span className="w-2 h-2 rounded-sm" style={{ background: '#62E4CC' }} />solar</span>
      ...
      {gen.map((g, i) => {
        ...
        return (
          <g key={i}>
            ...
            <rect x={x + bw * 0.5} y={H - gh} width={w} height={gh} rx={1.5} fill="#62E4CC" />
...
function DayCurve({ gen, load }: { gen: number[]; load: number[] }) {
  ...
  return (
    <svg viewBox={`0 0 ${W} ${H + 12}`} className="w-full" style={{ height: 'auto' }}>
      ...
      <polyline points={pts(gen)} fill="none" stroke="#62E4CC" strokeWidth="2" strokeLinejoin="round" />
...
function BoltMini() { return <svg width="13" height="13" viewBox="0 0 24 24" fill="none" stroke="#62E4CC" strokeWidth="2.2" strokeLinecap="round" strokeLinejoin="round"><path d="M13 2 3 14h7l-1 8 10-12h-7l1-8Z" /></svg> }
