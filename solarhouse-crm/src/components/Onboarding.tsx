...
export function Onboarding() {
  ...
  const finish = () => {
    ...
    act.toast(`Welcome — Solar House is set up for ${tradeByKey(trade).name.toLowerCase()}`)
  }
    ...
    <div className="fixed inset-0 z-[200] flex items-center justify-center p-6" style={{ background: 'rgba(11,18,32,0.55)', backdropFilter: 'blur(4px)' }}>
      <div className="w-full max-w-[840px] max-h-[90vh] overflow-y-auto bg-surface rounded-[20px] shadow-modal border border-border">
        ...
        <div className="px-8 pt-7 pb-5 border-b border-divider">
          ...
          <div className="text-[22px] font-bold text-ink tracking-[-0.01em]">{step === 0 ? 'What does your business do?' : `Great — let’s tailor Solar House for ${profile?.name}`}</div>
          <div className="text-[13.5px] text-muted-b mt-1">{step === 0 ? 'Pick your trade and Solar House sets up the right tools, job types, survey template and compliance checklist. You can change any of this later.' : 'These are switched on by default for your trade. Turn anything on or off — nothing is locked.'}</div>
