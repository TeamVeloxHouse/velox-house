      ...
      <div className="min-w-0 flex-1">
        ...
        {actionable && (
          <button onClick={onHandle} className="mt-2 h-8 px-3 rounded-lg text-[12.5px] font-semibold border border-border-blue bg-accent-wash text-accent flex items-center gap-1.5 hover:bg-[#E4ECFB] transition">
            <Sparkle size={13} /> Let Ovi handle this
          </button>
        )}
...
function Working({ steps, i }: { steps: string[]; i: number }) {
  return (
    <div className="flex gap-2.5 px-1">
      <MemberAvatar m={{ id: AI_MEMBER_ID, name: 'Ovi', role: '', color: '#1FAE94', status: 'online', bot: true }} size={34} />
      ...
      <div className="flex-1 flex flex-col min-w-0">
        ...
        <div className="h-[52px] shrink-0 border-b border-border bg-surface flex items-center gap-3 px-6">
          <div className="min-w-0">
            <div className="text-[15px] font-bold text-ink truncate flex items-center gap-2">{heading}{channel.ai && <Chip tone="accent" dot>Ovi is in here</Chip>}</div>
          </div>
        ...
        <div className="border-t border-border bg-canvas px-5 py-4">
          <div className="max-w-[780px] mx-auto">
            ...
            <div className="text-[11px] text-muted-3 text-center mt-2">{kindLabel} · {members.length} members{channel.ai && ' · Ovi can answer with live data and take action'}</div>
          </div>
        </div>
      </div>
    </div>
