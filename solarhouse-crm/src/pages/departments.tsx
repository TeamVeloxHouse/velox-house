  ...
  return (
    <div className="rounded-card border border-border bg-surface p-4">
      <div className="flex items-center gap-2.5 mb-1">
        ...
        <div className="flex-1"><div className="text-[14px] font-bold text-ink flex items-center gap-2">Ovi · {role} <Chip tone="positive" dot>Operator</Chip></div></div>
      </div>
  ...
  return (
    <DeptShell
      ...
      ai={<AiStrip role="Finance" blurb="Ask me for numbers and I’ll build the file — a live cashflow forecast or P&L as a real spreadsheet, a board deck from your pipeline, or I’ll chase every overdue invoice. Just like Claude in Excel, but wired to your live data."
        actions={[
          { label: 'Build cashflow forecast (.xlsx)', run: () => { buildCashflowWorkbook(state).then((b) => { downloadBlob(b, 'Solar House — 13-week cashflow.xlsx'); act.generateArtifact('Cashflow forecast — 13 week', 'model', 'xlsx') }) } },
          { label: 'Generate board pack (.pptx)', run: () => act.generateArtifact('Board pack — Q3 finance', 'deck', 'pptx') },
          { label: 'P&L this quarter (.xlsx)', run: () => { buildPnlWorkbook(state).then((b) => { downloadBlob(b, 'Solar House — P&L this quarter.xlsx'); act.generateArtifact('P&L — this quarter', 'report', 'xlsx') }) } },
          { label: 'Chase overdue invoices', run: () => { overdue.forEach((i) => act.addActivity({ type: 'email', subject: `Chase overdue invoice ${i.number} — ${i.customer}`, due: 'Today', priority: 'High', who: 'Ovi', source: 'ai' })); act.toast(`${overdue.length || 'No'} chase emails drafted`) } },
        ]} />}
    >
    ...
    <DeptShell
      ...
      ai={<AiStrip role="HR" blurb="I keep the people admin off your plate — draft contracts, letters and policies, keep the handbook current with the law, build onboarding checklists, and flag every certificate before it lapses."
        actions={[
          ...
          { label: 'Build onboarding checklist', run: () => { ['Send contract & handbook', 'Set up payroll & pension', 'Order PPE & tools', 'Book induction + H&S', 'Assign buddy'].forEach((t) => act.addActivity({ type: 'task', subject: `Onboarding — ${t}`, due: 'This week', priority: 'Medium', who: 'Ovi', source: 'ai' })); act.toast('Onboarding checklist added to tasks') } },
          { label: 'Chase expiring certs', run: () => { expiringCerts.forEach((c) => act.addActivity({ type: 'task', subject: `Renew ${c.name} — ${emp(c.employeeId)?.name}`, due: 'This week', priority: 'High', who: 'Ovi', source: 'ai' })); act.toast(`${expiringCerts.length} renewal tasks created`) } },
        ]} />}
    >
