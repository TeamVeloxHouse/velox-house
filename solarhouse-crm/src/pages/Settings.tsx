  ...
  return (
    <>
      <div>
        ...
        <div className="text-[13px] text-muted-b mt-0.5">Connect any email, calendar or social account. Solar House uses OAuth for Google &amp; Microsoft, generic IMAP/SMTP for everything else, and a unified social API — so any address works.</div>
      </div>
...
function ImapModal({ open, onClose, onConnect }: { open: boolean; onClose: () => void; onConnect: (email: string, protocol: 'imap') => void }) {
  ...
  return (
    <Modal open={open} onClose={onClose} title="Connect via IMAP / SMTP" subtitle="Works with any provider — Fastmail, Zoho, cPanel, custom domains" footer={<><Button onClick={onClose}>Cancel</Button><Button variant="primary" onClick={() => email.trim() && onConnect(email, 'imap')}>Connect</Button></>}>
      ...
      <div className="text-[12px] text-muted-2">In production this hands off to a secure OAuth or credential flow — no passwords touch Solar House’s servers in plain text.</div>
    </Modal>
  )
}
    ...
    <>
      <div className="flex items-center justify-between">
        <div>
          ...
          <div className="text-[13px] text-muted-b mt-0.5">Build your own integrations against the Solar House REST API, or push events anywhere with webhooks.</div>
        </div>
...
function AddWebhookModal({ open, onClose, onAdd }: { open: boolean; onClose: () => void; onAdd: (url: string, events: string[]) => void }) {
  ...
  return (
    <Modal open={open} onClose={onClose} title="Add webhook" subtitle="Solar House will POST the event payload to this URL" footer={<><Button onClick={onClose}>Cancel</Button><Button variant="primary" onClick={() => url.trim() && onAdd(url, events.split(',').map((s) => s.trim()).filter(Boolean))}>Add webhook</Button></>}>
...
function PlaybookModal({ initial, onClose, onSave }: { initial: Playbook | null; onClose: () => void; onSave: (data: { title: string; scope: PlaybookScope; body: string; source: 'written' | 'uploaded'; fileName?: string }) => void }) {
  ...
  const onFile = (f: File) => {
    ...
    else if (!body) setBody(`Uploaded ${f.name}. Ovi will read this file when it runs ${SCOPE_META[scope].label.toLowerCase()} tasks.`)
  }
  ...
  return (
    <Modal open onClose={onClose} title={initial ? 'Edit playbook' : 'Add playbook'} subtitle="Guidance the agents follow for a specific task" width={620}
      ...
      <div className="text-[11.5px] text-muted-3 leading-snug">Text files load inline; PDFs &amp; docs are stored and read by Ovi when the backend is connected.</div>
    </Modal>
  )
}
